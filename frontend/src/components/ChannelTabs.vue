  ...
  <q-toolbar>
    ...
    <div class="col q-mx-sm">
      ...
      <q-tabs
        ...
        @update:model-value="tabsStore.setActiveTab($event)"
        shrink
        stretch
        inline-label
        align="left"
      ...
      <q-select
        ...
        filled
        dense
        :options="tabsStore.tabs.map((tab) => ({ label: tab.label, value: tab.id }))"
        :model-value="tabsStore.activeTab"
        @update:model-value="tabsStore.setActiveTab($event)"
        label="Select Tab"
        map-options
      ...
      >
        ...
        <template v-slot:option="scope">
          <q-slide-item
            ref="slideItem"
            @left="(ev) => onSlide(ev, scope.opt.value)"
            @right="(ev) => onSlide(ev, scope.opt.value)"
            left-color="red"
            right-color="red"
          >
            ...
            <q-item clickable v-ripple @click="scope.toggleOption(scope.opt)">
              ...
              <!-- Optional close button (visible by default or on swipe) -->
              <q-item-section no-shadow side>
                <q-btn
                  ...
                  size="sm"
                  icon="close"
                  @click.stop="tabsStore.closeTab(scope.opt.value)"
                />
              </q-item-section>
            </q-item>
          </q-slide-item>
...
const onSlide = ({ reset }: SlideEvent, tab_id: string) => {
  reset();
  tabsStore.closeTab(tab_id);
};
